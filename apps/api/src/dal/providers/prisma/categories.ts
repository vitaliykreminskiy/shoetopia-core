...
import type { CategoriesDAL, Category, CategoryListParams, NavCategoryCount } from '../../category.js'
...
function mapRow(row: any): Category {
  return {
    ...
    computed_product_count: row.computed_count ?? row.computed_product_count ?? 0,
  }
}
...
export const prismaCategoriesDal: CategoriesDAL = {
  ...
  async list(params?: CategoryListParams) {
    ...
    try {
      ...
      return includeEmpty ? mapped : mapped.filter(c => c.computed_product_count > 0 || c.product_count > 0)
    } catch {
      return []
    }
  },
  ...
  async listChildren(parentSlug, minProductCount = 1) {
    try {
      ...
      `)
      return rows.map(r => ({ ...mapRow(r), computed_product_count: r.product_count ?? 0 }))
    } catch {
      return []
    }
  },
  ...
  async getNavCounts(country) {
    try {
      const rows = await prisma.$queryRawUnsafe<Array<{ gender: string; category: string; cnt: bigint }>>(
      ...
      return rows.map(r => ({ gender: r.gender, category: r.category, count: Number(r.cnt) }))
    } catch {
      return []
    }
  },
