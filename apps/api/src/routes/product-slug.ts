...
const productSlugRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    try {
      ...
      return reply.send({
        product: {
          pid: p.pid, slug: p.pid, name: p.name, brand: p.brand ?? p.manufacturer ?? null,
          advertiser_name: p.advertiserName ?? null, best_price: p.finalPrice ?? p.price,
          price: p.price, discount_pct: p.discount, is_on_sale: p.isOnSale ?? false,
          image_url: p.imageUrl, url: p.deepLinkURL ?? p.linkUrl,
          description: p.description ?? p.shortDescription,
          color: p.color, size: p.size, gender: p.gender, category: p.category,
        },
        source: 'flexoffers',
      })
    } catch (error) {
      fastify.log.error({ error }, '[products/:slug] error')
